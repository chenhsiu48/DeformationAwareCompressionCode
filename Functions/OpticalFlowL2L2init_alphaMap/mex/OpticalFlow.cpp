...
void OpticalFlow::SmoothFlowSOR(const DImage &Im1, const DImage &Im2, DImage &warpIm2, DImage &u, DImage &v,
	double &SumPhi, const DImage &alpha, int nOuterFPIterations, int nInnerFPIterations, int nSORIterations, double varepsilon_phi, double varepsilon_psi)
{
    ...
    double prob1,prob2,prob11,prob22;
    	//mexPrintf("%f \n",  beta);
    //double varepsilon_phi=pow(0.01,2);
    //double varepsilon_psi=pow(0.01,2);
    ...
    for(int count=0;count<nOuterFPIterations;count++)
	{
		...
		for(int hh=0;hh<nInnerFPIterations;hh++)
		{
			...
			for(int i=0;i<nPixels;i++)
			{
				...
				phiData[i] = 1;//0.5 / sqrt(temp + varepsilon_phi);
				//phiData[i] = 1/(power_alpha+temp);
				if (hh == nInnerFPIterations - 1) //Tamar
				{
					SumPhi_Temp += alpha[i]*temp;
				}
			}
			if (hh == nInnerFPIterations-1) //Tamar
			{
				SumPhi = SumPhi_Temp/nPixels; 
			}
			...
			for(int i=0;i<nPixels;i++)
			{
				imdtdx.data()[i] = -imdtdx.data()[i]-alpha[i]*foo1.data()[i];
				imdtdy.data()[i] = -imdtdy.data()[i]-alpha[i] *foo2.data()[i];
			}
				...
				for(int i = 0; i<imHeight; i++)
					for(int j = 0; j<imWidth; j++)
					{
						...
						if(j>0)
						{
							...
							sigma1  += -alpha[offset - 1] * _weight*du.data()[offset-1];
							sigma2  += -alpha[offset - 1] * _weight*dv.data()[offset-1];
							coeff   +=  alpha[offset - 1] * _weight;
						}
						if(j<imWidth-1)
						{
						...
						if(i>0)
						{
							...
							sigma1 += -alpha[offset - imWidth] * _weight*du.data()[offset-imWidth];
							sigma2 += -alpha[offset - imWidth] * _weight*dv.data()[offset-imWidth];
							coeff  +=  alpha[offset - imWidth] * _weight;
						}
						if(i<imHeight-1)
						{
                            _weight = phiData[offset];
						...
						du.data()[offset] = (1-omega)*du.data()[offset] + omega/(imdx2.data()[offset] + alpha[offset] *0.05 + coeff)*(imdtdx.data()[offset] - sigma1);
						// compute dv
						sigma2 += imdxy.data()[offset]*du.data()[offset];
						dv.data()[offset] = (1-omega)*dv.data()[offset] + omega/(imdy2.data()[offset] + alpha[offset] *0.05 + coeff)*(imdtdy.data()[offset] - sigma2);
					}
...
//--------------------------------------------------------------------------------------
// function to perfom ONE LEVEL optical flow estimation
//--------------------------------------------------------------------------------------
void OpticalFlow::Coarse2FineFlowTali(DImage &vx, DImage &vy, DImage &warpI2, const DImage &Im1, const DImage &Im2, double& SumPhi, DImage &alpha,
        int nOuterFPIterations, int nInnerFPIterations, int nCGIterations, double varepsilon_phi, double varepsilon_psi)
{
   ...
   im2feature(Image1,Im1);
   im2feature(Image2,Im2);
   ...
   if(interpolation == Bilinear)
    ...
    SmoothFlowSOR(Image1,Image2,WarpImage2,vx,vy,SumPhi,alpha,nOuterFPIterations,nInnerFPIterations,nCGIterations, varepsilon_phi, varepsilon_psi);
...
//--------------------------------------------------------------------------------------
// function to perfom coarse to fine optical flow estimation
//--------------------------------------------------------------------------------------
void OpticalFlow::Coarse2FineFlow(DImage &vx, DImage &vy, DImage &warpI2, const DImage &Im1, const DImage &Im2, double& SumPhi, DImage &alpha, double ratio, int minWidth,
	int nOuterFPIterations, int nInnerFPIterations, int nCGIterations, double varepsilon_phi, double varepsilon_psi)
{
	...
	for (int k = GPyramid1.nlevels() - 1;k >= 0;k--)
	{
		...
		SmoothFlowSOR(Image1, Image2, WarpImage2, vx, vy, SumPhi, alpha, nOuterFPIterations + k, nInnerFPIterations, nCGIterations + k * 3, varepsilon_phi, varepsilon_psi);
